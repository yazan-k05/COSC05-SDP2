"""
Compatibility IDS Server A (DDoS) proxy.

This legacy endpoint now forwards detection traffic to the new tiered stack
instead of running its own local legacy LLM pipeline.
"""
...
NODE_ID = os.getenv("NODE_ID", "IDS-A")
ZONE = os.getenv("ZONE", "A")
SPECIALTY = os.getenv("SPECIALTY", "ddos")
IDS_A_PORT = int(os.getenv("IDS_A_PORT", "8001"))
...
def health_check():
    ...
    return jsonify(
        {
            "status": "online",
            "service": "Compatibility IDS Server A",
            "node_id": NODE_ID,
            "zone": ZONE,
            "specialty": SPECIALTY,
...
if __name__ == "__main__":
    app.run(host="0.0.0.0", port=IDS_A_PORT, debug=False, threaded=True)
